...
class ModelArch(nn.Module):
    ...
    def forward(self, sent_id, mask):
        ...
        # use either first vec of last_hidden_state or pooler_output
        if self.args['model_type'] == 'xlm':
            cls_hs = self.hugg_model(sent_id, attention_mask=mask)[0]
            x = self.dropout1(cls_hs[:, 0, :])
        elif self.args['model_type'] in ('bert', 'xlmr'):
            # _, cls_hs = self.hugg_model(sent_id, attention_mask=mask, return_dict = False)
...
class ModelArch3(nn.Module):
    """
    transformers 3.4.0 and after, use this
    """
    def __init__(self, hugg_model, args):
        super(ModelArch3, self).__init__()
        self.hugg_model = hugg_model
        self.args = args
        if self.args['model_type'] == 'xlm':
            self.dropout1 = nn.Dropout(0.4)
